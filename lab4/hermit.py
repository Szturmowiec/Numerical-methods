...
def de(x):
	return m.sin(2.51327412287183/x)-2.51327412287183*m.cos(2.51327412287183/x)/x

def d(x):
	if len(x)==1: return ff(x[0])
	elif len(x)==2 and x[0]==x[1]: return de(x[0])
	else:
		x1=x.copy()
		del x1[0]
		x2=x.copy()
		del x2[len(x)-1]
		return (d(x1)-d(x2))/(x[len(x)-1]-x[0])
